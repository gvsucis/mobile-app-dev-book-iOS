...
class TraxyRepository {
    ...
    func listenForJournalUpdates(onChange: @escaping ([Journal]) -> Void) {
        self.listener = self.ref?.collection("journals").addSnapshotListener({ (snapshot, error) in
            ...
            for doc in documents {
                ...
                let coverPhotoUrl = doc["coverPhotoUrl"] as! String?

                let journal = Journal(key: key, name: name, location: location,
                        startDate: startDateStr?.dateFromISO8601, endDate: endDateStr?.dateFromISO8601, lat: lat, lng: lng, placeId: placeId, coverPhotoUrl: coverPhotoUrl)
    ...
    // MARK: Query methods
    func queryEntries(key: String, type: EntryType?, onCompletion: @escaping ([[String : Any]]?) -> Void)
    {
        if let uid = userId {
            let entries = Firestore.firestore().collection("user/\(uid)/journals/\(key)/entries")
            let query: Query
            if let t = type {
                query = entries.whereField("type", isEqualTo: t.rawValue)
                query.getDocuments() {(snapshot, err) in
                    if err != nil {
                        print("whoops query error")
                        onCompletion(nil)
                    } else {
                        var vals = [[String:Any]]()
                        for doc in snapshot!.documents {
                            vals.append(doc.data())
                        }
                        onCompletion(vals)
                    }
                }
            } else {
                entries.getDocuments() {(snapshot,err) in
                    if err != nil {
                        print("whoops query error")
                        onCompletion(nil)
                    } else {
